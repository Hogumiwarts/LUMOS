...
class GestureCache:
    ...
    # 사용자별 제스처 추가 메서드 (Few-shot learning)
    def add_user_gesture(self, user_id, gesture_name, samples, shots=10):
        """
        사용자별 커스텀 제스처 추가 (Few-shot learning)
        
        Args:
            user_id: 사용자 식별자
            gesture_name: 제스처 이름
            samples: 제스처 샘플 윈도우 리스트 (1초 길이)
            shots: 사용할 샘플 수
        """
        if len(samples) < shots:
            raise ValueError(f"최소 {shots}개 샘플이 필요합니다.")
        
        # 사용자 데이터 초기화 (필요시)
        if user_id not in self.user_gestures:
            self.user_gestures[user_id] = {
                "gestures": {},
                "mapping": {}
            }
        
        # 제스처 레이블 매핑
        if gesture_name not in self.user_gestures[user_id]["mapping"]:
            next_label = len(self.user_gestures[user_id]["mapping"]) + 100  # 기본 제스처와 구분
            self.user_gestures[user_id]["mapping"][gesture_name] = next_label
        
        label = self.user_gestures[user_id]["mapping"][gesture_name]
        
        # 샘플 제한
        samples = samples[:shots]
        
        # 데이터 증강
        augmented_samples = augment_data(samples)
        
        # 특징 추출 및 정규화
        features = [extract_combined_features(seq) for seq in augmented_samples]
        ...
        # 프로토타입 계산
        prototype = np.mean(features_scaled, axis=0)
        
        # 사용자 제스처 저장
        self.user_gestures[user_id]["gestures"][label] = {
            "name": gesture_name,
            "prototype": prototype,
