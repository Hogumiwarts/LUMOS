...
public interface AudioApiSpec {
    ...
    @Operation(summary = "스피커 전원 On/Off", description = "스피커의 전원을 제어 합니다.", tags = {"스피커"})
    @PatchMapping("/{deviceId}/audio/power")
    ResponseEntity<?> updateAudioPower(
    ...
    ResponseEntity<?> updateAudioVolume(
            @Parameter(description = "디바이스 ID", required = true)
            @PathVariable("deviceId") Long deviceId,
            @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    description = "회원 ID와 전원 상태 정보",
